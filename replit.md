...
- June 25, 2025. Enhanced Documents module with AI-powered document analysis using OpenAI GPT-4 Vision. System now automatically extracts document title, type, issue date, expiry date from uploaded images with confidence scoring. Smart reminder suggestions based on document type. Full-screen slide-in panel design matching other modules.
- June 25, 2025. Fixed document analysis form auto-population. System now properly extracts data from filename patterns (e.g., "2025.04.30 RSV Infotech Pte Ltd - Invoice.pdf") with 90% confidence, automatically populating title, document type (Purchase Invoice), issue date (2025-04-30), and due date (2025-05-30). Enhanced React Hook Form integration with shadcn Select components for proper field updates.
- June 26, 2025. Added comprehensive Payroll Management module with employee payroll configurations, automatic calculations for SGD currency, and payroll record processing. Features include base salary management, overtime calculations, Singapore CPF (20%) and tax deductions, allowances/deductions customization, and approval workflow (draft → pending → approved → paid). Supports monthly, bi-weekly, and weekly payroll periods with role-based access for HR managers and administrators.
- June 28, 2025. **Fixed Date Picker Navigation Issues** - Replaced complex calendar date pickers throughout the application with simple HTML date inputs. The previous calendar components were difficult to navigate (required clicking through months to go back years, inconsistent sizing, required precise mouse positioning). New date inputs provide native browser date selection with easy year/month navigation, consistent sizing, and better user experience. Updated CompanyDocumentForm, InvoiceForm, and other forms with SimpleDatePicker component.
